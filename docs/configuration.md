...
| `unfilled_timeout.sell.even_if_sell_signal_valid` | **Required.** Choose whether you would prefer your unfilled sell orders to [timeout even if sell signal was still valid](#timeout-even-if-sell-signal-valid). If false, your unfilled sell orders won't get timed out, so use this option wisely. <br>*Defaults to `true`.* <br> ***Datatype:*** *Boolean*
